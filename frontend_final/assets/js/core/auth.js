...
export async function fetchCurrentUser() {
  ...
  const session = getSession();
  const next = { ...(session || {}), user: payload.data.user, tokens: session?.tokens || null };
...
export async function registerUser(formData) {
  const payload = await request('/auth/register', { method: 'POST', auth: false, body: formData });
  saveAuthSession(payload.data);
  return payload.data;
}
...
export async function logoutUser({ redirect = true } = {}) {
  ...
  if (redirect) window.location.href = './login.html?message=Your%20explorer%20session%20has%20closed.';
}
...
setAuthFailureHandler((error) => {
  clearAuthSession();
  showToast({ type: 'warning', title: 'Session expired', message: error?.message || 'Please sign in again.' });
});
