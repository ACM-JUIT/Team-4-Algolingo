...
class _FakeEventSession:
    def __init__(self) -> None:
        self.added: list[UserEvent] = []
        self.flushed = False
        self.refreshed = False

    def add(self, instance: UserEvent) -> None:
        self.added.append(instance)

    async def flush(self) -> None:
        self.flushed = True

    async def refresh(self, instance: UserEvent) -> None:
        self.refreshed = True
        instance.id = uuid4()
        instance.created_at = datetime(2026, 1, 1, 0, 0, 0)
...
class _FakePracticeLookupSession:
    def __init__(self, value) -> None:
        self._value = value
...
class _FakeScalarOneResult:
    def __init__(self, *, scalar_one_value=None, scalar_one_or_none_value=None) -> None:
...
@pytest.mark.asyncio
async def test_event_service_refreshes_event_before_serialization() -> None:
    session = _FakeEventSession()

    event = await EventService.track_event(
        session=session,
        user_id=uuid4(),
        event_type="artifact_unlocked",
        event_data={"artifact": "Variable Vanguard"},
        commit=False,
    )

    assert session.flushed is True
    assert session.refreshed is True
    assert event.event_type == "artifact_unlocked"


@pytest.mark.asyncio
async def test_has_viewed_solution_handles_multiple_matching_events() -> None:
    session = _FakePracticeLookupSession(value=uuid4())

    result = await PracticeService.has_viewed_solution(
        session=session,
        user_id=uuid4(),
        practice_id=uuid4(),
    )
...
@pytest.mark.asyncio
async def test_get_quiz_questions_for_user_returns_questions_when_unlocked(monkeypatch) -> None:
    planet_id = uuid4()
    user_id = uuid4()
    question_id = uuid4()
    planet = Planet(
    ...
    session = _FakeQuizAccessSession(
        _FakeScalarOneResult(scalar_one_or_none_value=None),
        _FakeScalarOneResult(scalar_one_value=0),
    )

    async def fake_load_planet_with_quiz(*args, **kwargs):
        return planet

    async def fake_ensure_planet_is_unlocked(*args, **kwargs):
        return "UNLOCKED"

    def fake_determine_quiz_status(*args, **kwargs):
    ...
    monkeypatch.setattr(ProgressService, "ensure_planet_is_unlocked", fake_ensure_planet_is_unlocked)
    monkeypatch.setattr(ProgressService, "determine_quiz_status", fake_determine_quiz_status)
...
@pytest.mark.asyncio
async def test_get_quiz_questions_for_user_rejects_locked_quiz(monkeypatch) -> None:
    planet_id = uuid4()
    planet = Planet(
        id=planet_id,
        galaxy_id=uuid4(),
    ...
    with pytest.raises(AppException) as exc_info:
        await QuizService.get_quiz_questions_for_user(
            session=session,
            planet_id=planet_id,
            user_id=uuid4(),
        )
