...
def test_quiz_routes(client, monkeypatch):
    ...
    async def fake_get_quiz_questions(*args, **kwargs):
        return [
            QuizQuestionRead(
                id=question_id,
                planet_id=planet_id,
    ...
    monkeypatch.setattr(QuizService, "get_quiz_questions", fake_get_quiz_questions)
    ...
    get_response = client.get(f"/api/v1/planets/{planet_id}/quiz")
    ...
    assert get_response.status_code == 200
    assert submit_response.status_code == 200
    assert get_response.json()["data"][0]["question_text"] == "2 + 2?"
    assert submit_response.json()["data"]["passed"] is True
